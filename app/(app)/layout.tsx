...
const NAV = [
  ...
  { href: '/tesoreria/puertonoa-arg', label: 'Cta. cte. Puerto NOA Arg.', icon: '🇦🇷', soon: true },
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [tc, setTc] = useState({ ARS: null as number|null, CLP: null as number|null, CNY: null as number|null, fecha: '', hora: '', fuente: '' })
  ...
  async function loadTC() {
    try {
      const { data } = await supabase.from('tipos_cambio_eventos').select('ars,clp,cny,fecha,fuente,created_at').order('created_at', { ascending: false }).limit(10)
      if (!data || data.length === 0) return
      const r = { ARS: null as number|null, CLP: null as number|null, CNY: null as number|null, fecha: '', hora: '', fuente: '' }
      for (const ev of data as any[]) {
        if (r.ARS === null && ev.ars) { r.ARS = ev.ars; r.fecha = ev.fecha; r.hora = new Date(ev.created_at).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' }); r.fuente = ev.fuente === 'automatico' ? '🤖' : ev.fuente === 'forzado' ? '⚡' : '✏️' }
        if (r.CLP === null && ev.clp) r.CLP = ev.clp
        if (r.CNY === null && ev.cny) r.CNY = ev.cny
        if (r.ARS && r.CLP && r.CNY) break
      }
      setTc(r)
    } catch {}
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <aside className="w-56 flex-shrink-0 flex flex-col" style={{ background: 'linear-gradient(180deg, #052698 0%, #1168F8 100%)' }}>
        ...
        {/* TC Widget */}
        <div className="mx-3 mt-3 mb-1 rounded-xl" style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)' }}>
          <div className="px-3 py-1.5 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
            <span className="text-[10px] text-white/50 capitalize">{hoy}</span>
            <Link href="/tipos-cambio" className="text-[9px] text-white/30 hover:text-white/60">TC →</Link>
          </div>
          <div className="px-3 py-2 space-y-1">
            {[
              { m: 'ARS', f: '🇦🇷', v: tc.ARS, d: 0 },
              { m: 'CLP', f: '🇨🇱', v: tc.CLP, d: 0 },
              { m: 'CNY', f: '🇨🇳', v: tc.CNY, d: 4 },
            ].map(({ m, f, v, d }) => (
              <div key={m} className="flex items-center justify-between">
                <span className="text-[10px] text-white/40">{f} {m}</span>
                <span className="font-mono font-bold text-white text-[11px]">
                  {v !== null ? (d > 0 ? (v as number).toFixed(d) : Math.round(v as number).toLocaleString('es-AR')) : '—'}
                </span>
              </div>
            ))}
            {tc.fecha && <div className="text-[9px] text-white/25 pt-1" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>{tc.fuente} {tc.fecha} {tc.hora}</div>}
          </div>
        </div>
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          {NAV.map((item, i) => {
            ...
            if (!(item as any).href) return null
            const href = (item as any).href
            const active = pathname === href || pathname.startsWith(href + '/')
            if ((item as any).adminOnly && user?.rol !== 'admin') return null
            if ((item as any).soon) return (
              <div key={href} className="flex items-center gap-2 mx-2 px-3 py-1.5 rounded-lg text-[11px] mb-0.5 opacity-35 cursor-not-allowed">
                <span className="text-sm w-4 text-center flex-shrink-0 text-white/40">{(item as any).icon}</span>
            ...
            return (
              <Link key={href} href={href}
                ...
                <span className="text-sm w-4 text-center flex-shrink-0">{(item as any).icon}</span>
                <span className="truncate">{item.label}</span>
                {(item as any).adminOnly && <span className="ml-auto text-[7px] text-white/25 uppercase">Admin</span>}
              </Link>
            )
          })}
        </nav>
