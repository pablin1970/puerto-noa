...
export default function ClientesPage() {
  ...
  useEffect(() => { loadUser(); loadData(); loadRubros() }, [])
  ...
  async function loadRubros() {
    const { data } = await supabase.from('proveedor_rubros').select('*').eq('activo', true).order('orden')
  ...
  async function loadData() {
    ...
    const { data } = await supabase
      .from('terceros')
      .select('*, contactos:tercero_contactos(*), rubros:tercero_rubros(rubro_id, rubro:proveedor_rubros(id,nombre,color,icono))')
      .order('razon_social')
    if (data) setTerceros(data as any)
    setLoading(false)
  }
  ...
  const filtrados = terceros.filter(t => {
    ...
    const matchR = !filtroRubro || (t.rubros as any[])?.some((r: any) => r.rubro_id === filtroRubro)
    return matchB && matchT && matchP && matchR
  })

  const paises = Array.from(new Set(terceros.map(t => t.pais))).filter(Boolean).sort()
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          {view === 'lista' && (
            <button onClick={() => setView('nuevo')} className="px-5 py-2.5 bg-[#1168F8] text-white rounded-xl text-sm font-bold hover:bg-[#0a4fc4] shadow-sm">+ Nuevo</button>
          )}
        </div>
      </div>
        ...
        <>
          ...
          <div className="flex gap-3 mb-4 flex-wrap items-center">
            ...
            <select value={filtroRubro} onChange={e => setFiltroRubro(e.target.value)}
              ...
              {rubros.map(r => <option key={r.id} value={r.id}>{r.icono} {r.nombre}</option>)}
            </select>
            {(buscar || filtroTipo || filtroPais || filtroRubro) && (
              <button onClick={() => { setBuscar(''); setFiltroTipo(''); setFiltroPais(''); setFiltroRubro('') }}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                {terceros.length === 0 && <button onClick={() => setView('nuevo')} className="mt-3 px-4 py-2 bg-[#1168F8] text-white rounded-xl text-xs font-bold">+ Agregar primero</button>}
              </div>
            ) : (
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {['Razón social', 'Tipo', 'País', 'Documento', 'Rubros', 'Contacto principal', 'Ciudad', ''].map(h => (
                ...
                <tbody>
                  {filtrados.map(t => {
                    ...
                    const rubrosT = (t.rubros as any[] || []).map((r: any) => r.rubro).filter(Boolean)
                    return (
                      <tr key={t.id} className="border-b border-gray-50 hover:bg-blue-50/20 transition-colors group cursor-pointer"
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex gap-1 flex-wrap">
                            {rubrosT.slice(0, 2).map((r: any) => (
                              <span key={r.id} className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-semibold text-white" style={{ background: r.color }}>
                                {r.icono} {r.nombre}
                              </span>
                            ))}
                            {rubrosT.length > 2 && <span className="text-[9px] text-gray-400">+{rubrosT.length - 2}</span>}
                            {rubrosT.length === 0 && <span className="text-[10px] text-gray-300">—</span>}
                          </div>
                        </td>
                        <td className="px-4 py-3.5">
                          {contactoPpal && (
                        ...
                        <td className="px-4 py-3.5">
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={e => { e.stopPropagation(); setSelId(t.id); setView('detalle') }}
                              className="p-1.5 border border-gray-200 rounded-lg hover:bg-[#EBF2FF] text-gray-500 hover:text-[#1168F8]">✏</button>
                          </div>
                        </td>
                      </tr>
                    )
...
// ── FORMULARIO NUEVO — completo desde el inicio ────────────────
function FormTercero({ supabase, currentUser, todosRubros, onSave, onCancel }: any) {
  const [form, setForm] = useState({
    ...
    dir_fiscal_calle: '', dir_fiscal_ciudad: '', dir_fiscal_provincia: '',
    dir_fiscal_pais: 'Argentina', dir_fiscal_cp: '',
    notas: '', activo: true,
  ...
  function toggleRubro(id: string) {
    setRubrosSeleccionados(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])
  }

  function addContacto() {
    if (!newContacto.nombre) return
    setContactos(c => [...c, { ...newContacto, id: Date.now().toString() }])
    setNewContacto({ nombre: '', cargo: '', email: '', telefono: '', whatsapp: '', principal: false })
  }

  function addCuenta() {
    setCuentas(c => [...c, { banco: '', cuenta: '', cbu_iban: '', swift: '', moneda: 'USD', principal: c.length === 0, notas: '' }])
  }
  ...
  async function handleSave() {
    ...
    if (tercero) {
      ...
      if (contactos.length > 0) {
        await (supabase.from('tercero_contactos') as any).insert(
          contactos.map(({ id, ...c }) => ({ ...c, tercero_id: tercero.id }))
        )
      }
      // Cuentas bancarias
      const cuentasValidas = cuentas.filter(c => c.banco || c.cuenta || c.cbu_iban)
      if (cuentasValidas.length > 0) {
        await (supabase.from('tercero_cuentas_bancarias') as any).insert(
          cuentasValidas.map(c => ({ ...c, tercero_id: tercero.id }))
        )
      }
      // Rubros
      if (rubrosSeleccionados.length > 0) {
        await (supabase.from('tercero_rubros') as any).insert(
          rubrosSeleccionados.map(rId => ({ tercero_id: tercero.id, rubro_id: rId }))
        )
      }
    }
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Actividad comercial</label>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Calle y número</label>
          ...
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Provincia / Estado / Región</label>
            <input value={form.dir_fiscal_provincia} onChange={e => setForm(f => ({ ...f, dir_fiscal_provincia: e.target.value }))} className={inp} />
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">País fiscal</label>
            <select value={form.dir_fiscal_pais} onChange={e => setForm(f => ({ ...f, dir_fiscal_pais: e.target.value }))} className={inp}>
              {PAISES.map(p => <option key={p}>{p}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Código postal</label>
      ...
      {/* Contactos */}
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        <h3 className="font-bold text-sm text-gray-900 mb-4">Contactos</h3>
        <div className="grid grid-cols-3 gap-3 mb-3">
          <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Nombre</label>
            <input value={newContacto.nombre} onChange={e => setNewContacto(f => ({ ...f, nombre: e.target.value }))} className={inp} placeholder="Nombre completo" /></div>
          <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Cargo</label>
            <input value={newContacto.cargo} onChange={e => setNewContacto(f => ({ ...f, cargo: e.target.value }))} className={inp} placeholder="ej. Gerente comercial" /></div>
          ...
          <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Teléfono</label>
          ...
          <div className="flex items-end gap-3">
            ...
            <button onClick={addContacto} className="px-3 py-2 bg-[#1168F8] text-white rounded-xl text-xs font-bold hover:bg-[#0a4fc4] mb-0.5">+ Agregar</button>
          </div>
        </div>
        {contactos.length > 0 && (
          <div className="border border-gray-100 rounded-xl overflow-hidden mt-2">
            <table className="w-full text-xs">
              <thead><tr className="bg-gray-50">
                {['Nombre','Cargo','Email','Teléfono','WhatsApp',''].map(h => <th key={h} className="text-left px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase">{h}</th>)}
              </tr></thead>
              <tbody>
                {contactos.map((c, i) => (
                  <tr key={c.id} className="border-t border-gray-50">
                    <td className="px-3 py-2 font-medium text-gray-800">{c.nombre}{c.principal && <span className="ml-1 px-1.5 py-0.5 bg-[#EBF2FF] text-[#052698] rounded text-[9px] font-bold">Principal</span>}</td>
                    <td className="px-3 py-2 text-gray-500">{c.cargo || '—'}</td>
                    <td className="px-3 py-2 text-[#1168F8]">{c.email || '—'}</td>
                    <td className="px-3 py-2 font-mono text-[10px]">{c.telefono || '—'}</td>
                    <td className="px-3 py-2 font-mono text-[10px] text-green-700">{c.whatsapp || '—'}</td>
                    <td className="px-3 py-2"><button onClick={() => setContactos(prev => prev.filter((_, idx) => idx !== i))} className="text-gray-400 hover:text-red-500">✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        ...
        {cuentas.length === 0 ? (
          <div className="text-xs text-gray-400 text-center py-3 bg-gray-50 rounded-xl">Sin cuentas bancarias. Usá el botón para agregar.</div>
        ) : (
          <div className="space-y-3">
            {cuentas.map((c, i) => (
              <div key={i} className="border border-gray-100 rounded-xl p-4 bg-gray-50 relative">
                <button onClick={() => setCuentas(prev => prev.filter((_, idx) => idx !== i))} className="absolute top-3 right-3 text-gray-400 hover:text-red-500 text-xs">✕</button>
                <div className="grid grid-cols-3 gap-3">
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Banco</label><input value={c.banco} onChange={e => updateCuenta(i, 'banco', e.target.value)} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Moneda</label><select value={c.moneda} onChange={e => updateCuenta(i, 'moneda', e.target.value)} className={inp}>{MONEDAS.map(m => <option key={m}>{m}</option>)}</select></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">N° cuenta</label><input value={c.cuenta} onChange={e => updateCuenta(i, 'cuenta', e.target.value)} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">CBU / IBAN</label><input value={c.cbu_iban} onChange={e => updateCuenta(i, 'cbu_iban', e.target.value)} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">SWIFT / BIC</label><input value={c.swift} onChange={e => updateCuenta(i, 'swift', e.target.value)} className={inp} /></div>
      ...
      {/* Rubros — solo si es proveedor */}
      {form.tipo.includes('proveedor') && todosRubros.length > 0 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <h3 className="font-bold text-sm text-gray-900 mb-2">Rubros del proveedor</h3>
          <p className="text-[10px] text-gray-400 mb-4">Determina en qué bloque del cotizador aparece este proveedor</p>
          <div className="grid grid-cols-2 gap-2">
            {todosRubros.map((r: any) => {
              const sel = rubrosSeleccionados.includes(r.id)
              return (
                <button key={r.id} onClick={() => toggleRubro(r.id)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-left transition-all ${sel ? '' : 'border-gray-200 hover:border-gray-300 bg-gray-50'}`}
                  style={sel ? { background: r.color + '15', borderColor: r.color + '40' } : {}}>
                  <span className="text-lg">{r.icono}</span>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-semibold truncate" style={sel ? { color: r.color } : { color: '#374151' }}>{r.nombre}</div>
                    {r.descripcion && <div className="text-[10px] text-gray-400 truncate">{r.descripcion}</div>}
                  </div>
                  {sel && <span className="w-4 h-4 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0" style={{ background: r.color }}>✓</span>}
                </button>
              )
            })}
          </div>
...
function DetalleTercero({ tercero, supabase, currentUser, onReload, onBack }: any) {
  ...
  useEffect(() => {
    loadDocs(); loadOps(); loadCuentas(); loadRubros()
  }, [])
  ...
  async function saveData() {
    ...
    await (supabase.from('terceros') as any).update({
      razon_social: form.razon_social, nombre_fantasia: form.nombre_fantasia || null,
      pais: form.pais, tipo: tipoArray,
      tipo_doc: form.tipo_doc || null, nro_doc: form.nro_doc || null,
      condicion_iva: form.condicion_iva || null, actividad: form.actividad || null,
      nro_importador: form.nro_importador || null,
      dir_fiscal_calle: form.dir_fiscal_calle || null, dir_fiscal_ciudad: form.dir_fiscal_ciudad || null,
      dir_fiscal_provincia: form.dir_fiscal_provincia || null, dir_fiscal_pais: form.dir_fiscal_pais || null,
      dir_fiscal_cp: form.dir_fiscal_cp || null, notas: form.notas || null, activo: form.activo,
    }).eq('id', tercero.id)
    await onReload()
    setEditando(false)
    setSaving(false)
  ...
  async function subirDoc(file: File) {
    ...
    if (urlData?.publicUrl) {
      await (supabase.from('tercero_documentos') as any).insert({
        tercero_id: tercero.id, tipo: docForm.tipo,
        nombre_custom: docForm.tipo === 'otro' ? docForm.nombre_custom : null,
        referencia: docForm.referencia || null, fecha: docForm.fecha || null, notas: docForm.notas || null,
        archivo_url: urlData.publicUrl, archivo_nombre: file.name, subido_por: currentUser?.nombre,
      })
      await loadDocs()
    }
    setUploading(false)
  ...
  const TIPOS_DOC_LABEL: Record<string, string> = {
    estatuto: 'Estatuto / Acta constitutiva', poder: 'Poder notarial',
    certificado: 'Certificado / Habilitación', rut: 'RUT / CUIT / Constancia fiscal',
    contrato: 'Contrato marco', otro: 'Otro',
  }
  const tiposDocs = TIPO_DOC_POR_PAIS[form.pais] || TIPO_DOC_POR_PAIS.default
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {[
          ...
          { key: 'rubros',     label: `Rubros (${rubros.length})` },
        ].map(t => (
          ...
          {editando ? (
            <>
              <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                ...
                <div className="flex gap-3">
                  {[{ key: 'cliente', label: 'Cliente', icon: '🤝', desc: 'Contrata servicios a Puerto NOA' }, { key: 'proveedor', label: 'Proveedor', icon: '📦', desc: 'Presta servicios a Puerto NOA' }].map(o => {
                    const tipoActual = Array.isArray(form.tipo) ? form.tipo : []
                    const sel = tipoActual.includes(o.key)
                    return (
                      <button key={o.key} onClick={() => {
                        const nuevos = sel ? tipoActual.filter((x: string) => x !== o.key) : [...tipoActual, o.key]
                        if (nuevos.length === 0) return
                        setForm((f: any) => ({ ...f, tipo: nuevos }))
                      }} className={`flex-1 px-4 py-3 rounded-xl border-2 text-left transition-all ${sel ? 'border-[#1168F8] bg-[#EBF2FF]' : 'border-gray-200 hover:bg-gray-50'}`}>
                        <div className="text-base mb-1">{o.icon}</div>
                        <div className="font-bold text-sm text-gray-900">{o.label}</div>
                        <div className="text-[10px] text-gray-400">{o.desc}</div>
                      </button>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                <h3 className="font-bold text-sm text-gray-900 mb-4">Datos generales</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2"><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Razón social</label><input value={form.razon_social} onChange={e => setForm((f: any) => ({ ...f, razon_social: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Nombre fantasía</label><input value={form.nombre_fantasia || ''} onChange={e => setForm((f: any) => ({ ...f, nombre_fantasia: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">País</label><select value={form.pais} onChange={e => setForm((f: any) => ({ ...f, pais: e.target.value }))} className={inp}>{PAISES.map(p => <option key={p}>{p}</option>)}</select></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Tipo documento</label><select value={form.tipo_doc || ''} onChange={e => setForm((f: any) => ({ ...f, tipo_doc: e.target.value }))} className={inp}>{tiposDocs.map(d => <option key={d}>{d}</option>)}</select></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Número</label><input value={form.nro_doc || ''} onChange={e => setForm((f: any) => ({ ...f, nro_doc: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Condición IVA</label><select value={form.condicion_iva || ''} onChange={e => setForm((f: any) => ({ ...f, condicion_iva: e.target.value }))} className={inp}>{CONDICION_IVA.map(c => <option key={c}>{c}</option>)}</select></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">N° importador</label><input value={form.nro_importador || ''} onChange={e => setForm((f: any) => ({ ...f, nro_importador: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Actividad</label><input value={form.actividad || ''} onChange={e => setForm((f: any) => ({ ...f, actividad: e.target.value }))} className={inp} /></div>
                </div>
              </div>
              <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                <h3 className="font-bold text-sm text-gray-900 mb-4">Dirección fiscal</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2"><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Calle y número</label><input value={form.dir_fiscal_calle || ''} onChange={e => setForm((f: any) => ({ ...f, dir_fiscal_calle: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Ciudad</label><input value={form.dir_fiscal_ciudad || ''} onChange={e => setForm((f: any) => ({ ...f, dir_fiscal_ciudad: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Provincia / Región</label><input value={form.dir_fiscal_provincia || ''} onChange={e => setForm((f: any) => ({ ...f, dir_fiscal_provincia: e.target.value }))} className={inp} /></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">País fiscal</label><select value={form.dir_fiscal_pais || form.pais} onChange={e => setForm((f: any) => ({ ...f, dir_fiscal_pais: e.target.value }))} className={inp}>{PAISES.map(p => <option key={p}>{p}</option>)}</select></div>
                  <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Código postal</label><input value={form.dir_fiscal_cp || ''} onChange={e => setForm((f: any) => ({ ...f, dir_fiscal_cp: e.target.value }))} className={inp} /></div>
                </div>
              </div>
              <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                <h3 className="font-bold text-sm text-gray-900 mb-3">Notas</h3>
                <textarea value={form.notas || ''} onChange={e => setForm((f: any) => ({ ...f, notas: e.target.value }))} className={inp + ' resize-none'} rows={2} />
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={() => setEditando(false)} className="px-4 py-2 border border-gray-200 rounded-xl text-xs">Cancelar</button>
                <button onClick={saveData} disabled={saving} className="px-5 py-2 bg-[#1168F8] text-white rounded-xl text-xs font-bold disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar cambios'}</button>
              </div>
            </>
          ) : (
            <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
              <div className="grid grid-cols-3 gap-x-8 gap-y-4">
                {[
                  ...
                  { l: 'Provincia / Región', v: tercero.dir_fiscal_provincia },
                  { l: 'País fiscal', v: tercero.dir_fiscal_pais },
                  { l: 'Código postal', v: tercero.dir_fiscal_cp },
                ].filter(r => r.v).map(r => (
                  <div key={r.l}>
      ...
      {tab === 'contactos' && (
        <div className="space-y-4">
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Nombre *</label><input value={newContacto.nombre} onChange={e => setNewContacto(f => ({ ...f, nombre: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Cargo</label><input value={newContacto.cargo} onChange={e => setNewContacto(f => ({ ...f, cargo: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Email</label><input type="email" value={newContacto.email} onChange={e => setNewContacto(f => ({ ...f, email: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Teléfono</label><input value={newContacto.telefono} onChange={e => setNewContacto(f => ({ ...f, telefono: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">WhatsApp</label><input value={newContacto.whatsapp} onChange={e => setNewContacto(f => ({ ...f, whatsapp: e.target.value }))} className={inp} /></div>
              <div className="flex items-end"><label className="flex items-center gap-2 cursor-pointer pb-2"><input type="checkbox" checked={newContacto.principal} onChange={e => setNewContacto(f => ({ ...f, principal: e.target.checked }))} className="w-4 h-4 rounded" /><span className="text-xs text-gray-600 font-medium">Principal</span></label></div>
            </div>
            <div className="flex justify-end"><button onClick={addContacto} className="px-4 py-2 bg-[#1168F8] text-white rounded-xl text-xs font-bold">+ Agregar</button></div>
          </div>
          {contactos.length > 0 && (
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
              <table className="w-full text-xs">
                <thead><tr className="bg-gray-50 border-b border-gray-100">{['Nombre','Cargo','Email','Teléfono','WhatsApp',''].map(h => <th key={h} className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
                <tbody>
                  {contactos.map(c => (
                    <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3"><button onClick={() => deleteContacto(c.id)} className="text-gray-400 hover:text-red-500">✕</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Banco</label><input value={newCuenta.banco} onChange={e => setNewCuenta(f => ({ ...f, banco: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Moneda</label><select value={newCuenta.moneda} onChange={e => setNewCuenta(f => ({ ...f, moneda: e.target.value }))} className={inp}>{MONEDAS.map(m => <option key={m}>{m}</option>)}</select></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">N° cuenta</label><input value={newCuenta.cuenta} onChange={e => setNewCuenta(f => ({ ...f, cuenta: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">CBU / IBAN</label><input value={newCuenta.cbu_iban} onChange={e => setNewCuenta(f => ({ ...f, cbu_iban: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">SWIFT / BIC</label><input value={newCuenta.swift} onChange={e => setNewCuenta(f => ({ ...f, swift: e.target.value }))} className={inp} /></div>
              <div className="flex items-end"><label className="flex items-center gap-2 cursor-pointer pb-2"><input type="checkbox" checked={newCuenta.principal} onChange={e => setNewCuenta(f => ({ ...f, principal: e.target.checked }))} className="w-4 h-4 rounded" /><span className="text-xs text-gray-600 font-medium">Cuenta principal</span></label></div>
            </div>
            <div className="flex justify-end"><button onClick={addCuenta} className="px-4 py-2 bg-[#1168F8] text-white rounded-xl text-xs font-bold">+ Agregar</button></div>
          </div>
          {cuentas.length > 0 ? (
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
              <table className="w-full text-xs">
                <thead><tr className="bg-gray-50 border-b border-gray-100">{['Banco','Moneda','N° Cuenta','CBU / IBAN','SWIFT',''].map(h => <th key={h} className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
                <tbody>
                  {cuentas.map(c => (
                    <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3"><button onClick={() => deleteCuenta(c.id)} className="text-gray-400 hover:text-red-500">✕</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            ...
            <div className="grid grid-cols-4 gap-3 mb-3">
              ...
              {docForm.tipo === 'otro' && <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Nombre *</label><input value={docForm.nombre_custom} onChange={e => setDocForm(f => ({ ...f, nombre_custom: e.target.value }))} className={inp} /></div>}
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Referencia</label><input value={docForm.referencia} onChange={e => setDocForm(f => ({ ...f, referencia: e.target.value }))} className={inp} /></div>
              <div><label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Fecha</label><input type="date" value={docForm.fecha} onChange={e => setDocForm(f => ({ ...f, fecha: e.target.value }))} className={inp} /></div>
            </div>
            <label className={`flex items-center gap-2 px-4 py-2.5 border-2 border-dashed border-[#93B8FC] rounded-xl text-xs text-[#1168F8] hover:bg-[#EBF2FF] cursor-pointer w-fit ${uploading ? 'opacity-60' : ''}`}>
              📎 {uploading ? 'Subiendo...' : 'Seleccionar y subir archivo'}
              <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" disabled={uploading} onChange={e => { const f = e.target.files?.[0]; if (f) subirDoc(f) }} />
            </label>
          </div>
          {docs.length > 0 && (
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
              <div className="divide-y divide-gray-50">
                {docs.map((d: any) => (
                  <div key={d.id} className="flex items-center gap-4 px-5 py-3.5">
                    ...
                    {d.archivo_url && <a href={d.archivo_url} target="_blank" rel="noreferrer" className="px-3 py-1.5 bg-[#EBF2FF] text-[#1168F8] rounded-lg text-xs font-medium hover:bg-[#93B8FC]">Ver</a>}
                  </div>
                ))}
              </div>
            </div>
      ...
      {tab === 'operaciones' && (
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
          {ops.length === 0 ? <div className="p-8 text-center text-gray-400 text-sm">Sin operaciones vinculadas.</div> : (
            <table className="w-full text-xs">
              <thead><tr className="bg-gray-50 border-b border-gray-100">{['N° Cotización','Cliente','Estado','Fecha'].map(h => <th key={h} className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
              <tbody>
                {ops.map((o: any) => (
      ...
      {tab === 'rubros' && (
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <div className="mb-4">
            ...
            <div className="text-[10px] text-gray-400">Determina en qué bloque del cotizador aparece este proveedor</div>
          </div>
              ...
              {todosRubros.map(r => {
                ...
                return (
                  <button key={r.id} onClick={() => toggleRubro(r.id)}
                    ...
                    {asignado && <span className="w-4 h-4 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0" style={{ background: r.color }}>✓</span>}
                  </button>
                )
              })}
            </div>
