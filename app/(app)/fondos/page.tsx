  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        <h3 className="font-semibold text-sm text-gray-900 mb-4">Registrar movimiento</h3>

        {/* Tipo de movimiento */}
        <div className="grid grid-cols-4 gap-2 mb-4">
          {TIPOS_MOV.map(t => (
            <button key={t.key} onClick={() => setForm(f => ({ ...f, tipo: t.key }))}
              className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border-2 text-left transition-all ${form.tipo === t.key ? `${t.bg} ${t.border} ${t.color} border-2` : 'border-gray-200 hover:bg-gray-50 text-gray-600'}`}>
        ...
        <div className="grid grid-cols-4 gap-3 mb-3">
          ...
          <div className="col-span-2">
            ...
            <input value={form.concepto} onChange={e => setForm(f => ({ ...f, concepto: e.target.value }))} className={inp}
              placeholder={form.tipo === 'ingreso_cliente' ? 'ej. Anticipo operación' : form.tipo === 'pago_proveedor' ? 'ej. Pago flete Triunfo SRL' : form.tipo === 'honorarios_puertonoa' ? 'ej. Honorarios Puerto NOA COT-2026-001' : 'ej. Traspaso caja a banco'}/>
          </div>
          <div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-3">
          <div>
            ...
            <select value={form.moneda} onChange={e => setForm(f => ({ ...f, moneda: e.target.value, cuenta_id: '', cuenta_destino_id: '' }))} className={inp}>
          ...
          {form.moneda !== 'USD' && (
            <div>
              <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">
                TC {form.moneda}/USD
                <span className="text-gray-300 ml-1 font-normal normal-case">(auto: {Math.round(tcAuto).toLocaleString('es-AR')})</span>
              </label>
              <input type="text" inputMode="decimal" value={form.tc_usd} onFocus={e => e.target.select()}
                onChange={e => setForm(f => ({ ...f, tc_usd: e.target.value }))}
                className={inp + ' text-right font-mono'} placeholder={Math.round(tcAuto).toString()}/>
            </div>
          )}
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Equivalente USD</label>
            <div className="px-3 py-2 bg-[#EBF2FF] border border-[#93B8FC] rounded-xl text-xs font-mono font-bold text-[#052698] text-right">
              USD {fmt(usdEquiv, 2)}
            </div>
          </div>
        </div>
        ...
        {/* Cuentas */}
        <div className={`grid gap-3 mb-3 ${form.tipo === 'transferencia' ? 'grid-cols-2' : 'grid-cols-1'}`}>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">
            ...
            <select value={form.cuenta_id} onChange={e => setForm(f => ({ ...f, cuenta_id: e.target.value }))} className={inp}>
              ...
              {cuentasCompatibles.map((c: any) => (
                <option key={c.id} value={c.id}>{c.pais === 'Argentina' ? '🇦🇷' : '🇨🇱'} {c.nombre} ({c.moneda})</option>
              ))}
            </select>
          </div>
          {form.tipo === 'transferencia' && (
            <div>
              ...
              <select value={form.cuenta_destino_id} onChange={e => setForm(f => ({ ...f, cuenta_destino_id: e.target.value }))} className={inp}>
                <option value="">— Seleccionar cuenta —</option>
                {cuentas.filter((c: any) => c.id !== form.cuenta_id).map((c: any) => (
                  <option key={c.id} value={c.id}>{c.pais === 'Argentina' ? '🇦🇷' : '🇨🇱'} {c.nombre} ({c.moneda})</option>
                ))}
              </select>
            </div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-3">
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Banco origen</label>
            <input value={form.banco_origen} onChange={e => setForm(f => ({ ...f, banco_origen: e.target.value }))} className={inp} placeholder="Banco del remitente"/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Cuenta / CBU origen</label>
            <input value={form.cuenta_origen} onChange={e => setForm(f => ({ ...f, cuenta_origen: e.target.value }))} className={inp} placeholder="N° cuenta origen"/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Banco destino</label>
            <input value={form.banco_destino} onChange={e => setForm(f => ({ ...f, banco_destino: e.target.value }))} className={inp} placeholder="Banco receptor"/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Cuenta / CBU destino</label>
            <input value={form.cuenta_destino_texto} onChange={e => setForm(f => ({ ...f, cuenta_destino_texto: e.target.value }))} className={inp} placeholder="N° cuenta destino"/>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">N° referencia / transferencia</label>
            <input value={form.nro_referencia} onChange={e => setForm(f => ({ ...f, nro_referencia: e.target.value }))} className={inp} placeholder="Comprobante, N° operación bancaria"/>
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Comprobante (PDF / imagen)</label>
            <div className="flex items-center gap-2">
              <label className="flex items-center gap-1.5 px-3 py-2 border border-dashed border-gray-300 rounded-xl text-xs text-gray-500 hover:border-[#1168F8] hover:text-[#1168F8] cursor-pointer">
        ...
        <div className="mb-4">
          ...
          <input value={form.notas} onChange={e => setForm(f => ({ ...f, notas: e.target.value }))} className={inp} placeholder="Observaciones adicionales"/>
        </div>
        ...
        {form.tipo === 'honorarios_puertonoa' && (
          <div className="mb-4 px-4 py-3 bg-[#EBF2FF] border border-[#93B8FC] rounded-xl text-[11px] text-[#052698]">
            💡 Este movimiento registrará un egreso en los fondos a rendir. Recordá también registrar el ingreso correspondiente en la caja propia de Puerto NOA.
          </div>
        )}
