...
export default function FacturasEmitidasPage() {
  ...
  async function loadData() {
    ...
    const [fRes, tRes, oRes] = await Promise.all([
      ...
      supabase.from('terceros').select('id,razon_social,nro_doc,tipo_doc,cliente_giro:actividad,cliente_direccion:dir_fiscal_calle,cliente_ciudad:dir_fiscal_ciudad,cliente_pais:pais').contains('tipo', ['cliente']),
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          {view !== 'lista' && (
            <button onClick={() => { setView('lista'); setSelId(null) }}
              className="px-4 py-2 border border-gray-200 rounded-xl text-xs font-semibold hover:bg-gray-100">← Volver</button>
          )}
          {view === 'lista' && (
          ...
          {view === 'detalle' && sel && (
            <button onClick={() => setView('impresion')}
              className="px-4 py-2 bg-[#052698] text-white rounded-xl text-xs font-bold hover:bg-[#1168F8]">
              🖨 Imprimir / PDF
            </button>
          )}
        </div>
        ...
        <>
          ...
          <div className="flex gap-3 mb-4 flex-wrap items-center">
            <div className="relative flex-1 min-w-60">
              ...
              <input value={buscar} onChange={e => setBuscar(e.target.value)} placeholder="Buscar cliente, folio, operación..."
            ...
            {(buscar || filtroEstado) && (
              <button onClick={() => { setBuscar(''); setFiltroEstado('') }}
                className="px-3 py-2 border border-gray-200 rounded-xl text-xs text-gray-500 hover:bg-gray-50">✕</button>
            )}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
            {loading ? (
              ...
              <div className="p-12 text-center">
                ...
                <button onClick={() => setView('nueva')} className="mt-3 px-4 py-2 bg-[#1168F8] text-white rounded-xl text-xs font-bold">
                  ...
                  {filtradas.map(f => (
                    <tr key={f.id} className="border-b border-gray-50 hover:bg-blue-50/20 transition-colors group cursor-pointer"
                      ...
                      <td className="px-4 py-3.5">
                        <div className="font-mono font-bold text-gray-900">{f.folio ? `#${f.folio}` : <span className="text-gray-300">Sin folio</span>}</div>
                      </td>
                      <td className="px-4 py-3.5">
                        <span className="text-xs text-gray-600">{TIPO_DOC_L[f.tipo_doc] || f.tipo_doc}</span>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        {f.cotizacion_num ? (
                          <span className="font-mono text-[11px] text-[#1168F8]">{f.cotizacion_num}</span>
                        ) : <span className="text-gray-300">—</span>}
                      </td>
                      <td className="px-4 py-3.5 font-mono text-[11px] text-gray-600">{f.fecha_emision}</td>
                      <td className="px-4 py-3.5">
                        <div className="font-mono font-bold text-gray-900">
                          {f.moneda} {f.moneda === 'CLP' ? Math.round(f.total).toLocaleString('es-CL') : fmt(f.total, 2)}
                        </div>
                        {f.total_usd && <div className="text-[10px] text-gray-400 font-mono">USD {fmt(f.total_usd, 0)}</div>}
                      </td>
                      <td className="px-4 py-3.5">
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-semibold border ${ESTADO_CLS[f.estado]}`}>
                          {ESTADO_L[f.estado]}
                        </span>
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="opacity-0 group-hover:opacity-100 flex gap-1">
                          <button className="p-1.5 border border-gray-200 rounded-lg hover:bg-[#EBF2FF] text-gray-500 hover:text-[#1168F8] text-xs">👁</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {view === 'detalle' && sel && (
        <DetalleFactura
          ...
          supabase={supabase}
          onReload={loadData}
          onImprimir={() => setView('impresion')}
          onBack={() => setView('lista')}
        />
      )}

      {view === 'impresion' && sel && (
        <ImpresionFactura factura={sel} onBack={() => setView('detalle')} />
      )}
    </div>
  )
}
...
function FormFactura({ supabase, currentUser, terceros, operaciones, onSave, onCancel }: any) {
  const [form, setForm] = useState({
    ...
    iva_pct: 19,
    tipo_cobro: 'servicio',
    discriminar_items: true,
    glosa: '',
    notas_internas: '',
  })
  const [items, setItems] = useState([
    { descripcion: '', cantidad: 1, precio_unit: 0, descuento: 0, exento: false }
  ])
  ...
  function selectTercero(t: any) {
    setForm(f => ({
      ...f,
      tercero_id: t.id,
      cliente_razon_social: t.razon_social,
      cliente_rut: t.nro_doc || '',
      cliente_direccion: t.dir_fiscal_calle || '',
      cliente_ciudad: t.dir_fiscal_ciudad || '',
      cliente_pais: t.pais || 'Chile',
      cliente_giro: t.actividad || '',
    }))
    setBuscarTercero(t.razon_social)
    setShowTerceroDD(false)
  }
  ...
  async function guardar() {
    ...
    setSaving(true)
    const itemsLimpios = items.filter(i => i.descripcion).map(i => {
      const c = calcItem(i)
      return { ...i, neto: c.neto, iva_monto: c.iva, exento_monto: c.exento, total: c.total }
    })
    const tcRef = parseFloat(form.tc_referencia as any) || null
    await (supabase.from('facturas_emitidas') as any).insert({
      ...form,
      tc_referencia: tcRef,
      iva_pct: form.iva_pct,
      items: itemsLimpios,
      neto: Math.round(totales.neto),
      iva_monto: Math.round(totales.iva),
      exento: Math.round(totales.exento),
    ...
    <div className="max-w-4xl space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          {form.moneda !== 'CLP' && (
            <div>
              <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">TC referencia (por USD)</label>
              <input type="text" value={form.tc_referencia} onChange={e => setForm(f => ({ ...f, tc_referencia: e.target.value }))} className={inp} placeholder="ej. 950" />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
            ...
            {showTerceroDD && (
              <div className="absolute z-50 top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-xl max-h-40 overflow-y-auto mt-1">
                {terceros.filter((t: any) => t.razon_social.toLowerCase().includes((buscarTercero || form.cliente_razon_social).toLowerCase())).slice(0, 6).map((t: any) => (
                  <button key={t.id} onMouseDown={() => selectTercero(t)}
                    className="w-full text-left px-4 py-2.5 hover:bg-[#EBF2FF] border-b border-gray-50 last:border-0">
          ...
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">RUT / CUIT</label>
            <input value={form.cliente_rut} onChange={e => setForm(f => ({ ...f, cliente_rut: e.target.value }))} className={inp} placeholder="XX.XXX.XXX-X" />
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Giro</label>
            <input value={form.cliente_giro} onChange={e => setForm(f => ({ ...f, cliente_giro: e.target.value }))} className={inp} placeholder="Actividad comercial" />
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Dirección</label>
            <input value={form.cliente_direccion} onChange={e => setForm(f => ({ ...f, cliente_direccion: e.target.value }))} className={inp} />
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Ciudad</label>
            <input value={form.cliente_ciudad} onChange={e => setForm(f => ({ ...f, cliente_ciudad: e.target.value }))} className={inp} />
          </div>
        </div>

        {/* Vinculación a operación */}
        <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-gray-100">
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Operación vinculada</label>
            <select value={form.operacion_id} onChange={e => {
              const op = operaciones.find((o: any) => o.id === e.target.value)
              setForm(f => ({ ...f, operacion_id: e.target.value, cotizacion_num: op?.cotizacion?.num || '' }))
            }} className={inp}>
          ...
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Tipo de cobro</label>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        ...
        <table className="w-full text-xs mb-3">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="text-left px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase">Descripción</th>
              <th className="text-right px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase w-16">Cant.</th>
              <th className="text-right px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase w-28">Precio unit.</th>
              <th className="text-right px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase w-16">Desc. %</th>
              <th className="text-center px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase w-16">Exento</th>
              <th className="text-right px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase w-28">Total</th>
              <th className="w-8"></th>
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={i} className="border-b border-gray-50">
                  <td className="px-2 py-2">
                    <input value={item.descripcion} onChange={e => { const n = [...items]; n[i] = { ...n[i], descripcion: e.target.value }; setItems(n) }}
                      className="w-full px-2 py-1.5 border border-transparent rounded-lg hover:border-gray-200 focus:border-[#1168F8] focus:outline-none text-xs" placeholder="Descripción del servicio..." />
                  </td>
                  <td className="px-2 py-2">
                  ...
                  <td className="px-2 py-2">
                    <input type="number" value={item.descuento} onChange={e => { const n = [...items]; n[i] = { ...n[i], descuento: parseFloat(e.target.value) || 0 }; setItems(n) }}
                      className="w-full px-2 py-1.5 border border-transparent rounded-lg hover:border-gray-200 focus:border-[#1168F8] focus:outline-none text-xs text-right" min="0" max="100" />
                  </td>
                  ...
                  <td className="px-2 py-2">
                    {items.length > 1 && (
                      <button onClick={() => setItems(items.filter((_, j) => j !== i))} className="text-gray-300 hover:text-red-500 text-xs">✕</button>
                    )}
                  </td>
                </tr>
              )
        ...
        <button onClick={() => setItems([...items, { descripcion: '', cantidad: 1, precio_unit: 0, descuento: 0, exento: false }])}
          className="text-xs text-[#1168F8] hover:underline">+ Agregar ítem</button>
            ...
            <div className="w-64 space-y-1.5 text-xs">
              {totales.neto > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Neto afecto</span>
                  <span className="font-mono">{fmtCLP(totales.neto)}</span>
                </div>
              )}
              {totales.exento > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Exento</span>
                  <span className="font-mono">{fmtCLP(totales.exento)}</span>
                </div>
              )}
              ...
              {form.tc_referencia && (
                <div className="flex justify-between text-[10px] text-gray-400">
                  <span>Ref. USD (TC {form.tc_referencia})</span>
                  <span className="font-mono">USD {fmt(totales.total / parseFloat(form.tc_referencia as any), 0)}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <textarea value={form.glosa} onChange={e => setForm(f => ({ ...f, glosa: e.target.value }))}
              className={inp + ' resize-none'} rows={2} placeholder="Observaciones que aparecen en el documento impreso..." />
          </div>
          <div>
            <label className="block text-[10px] font-semibold text-gray-500 mb-1 uppercase">Notas internas (no aparecen)</label>
            <textarea value={form.notas_internas} onChange={e => setForm(f => ({ ...f, notas_internas: e.target.value }))}
              className={inp + ' resize-none'} rows={2} placeholder="Uso interno..." />
          </div>
        </div>
      </div>
...
function DetalleFactura({ factura, supabase, onReload, onImprimir, onBack }: any) {
  ...
  async function guardarFolio() {
    ...
    await (supabase.from('facturas_emitidas') as any).update({
      folio: parseInt(folio) || null,
      estado: parseInt(folio) ? 'emitida' : factura.estado,
    }).eq('id', factura.id)
    await onReload()
    ...
    <div className="max-w-3xl space-y-4">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="text-xl font-black font-mono text-[#052698]">
                {factura.folio ? `#${factura.folio}` : <span className="text-gray-300 text-sm">Sin folio SII</span>}
              </div>
              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${ESTADO_CLS[factura.estado]}`}>
                {ESTADO_L[factura.estado]}
              </span>
            </div>
          ...
          <div className="text-right">
            <div className="text-2xl font-black font-mono text-[#052698]">
              {factura.moneda} {Math.round(factura.total).toLocaleString('es-CL')}
            </div>
            ...
            {editandoFolio ? (
              <div className="flex items-center gap-2">
                <input value={folio} onChange={e => setFolio(e.target.value)} type="number"
                  className="w-24 px-2 py-1 border border-[#1168F8] rounded-lg text-xs font-mono text-center focus:outline-none" />
                <button onClick={guardarFolio} disabled={saving} className="px-3 py-1 bg-[#1168F8] text-white rounded-lg text-xs font-bold">
                  {saving ? '...' : '✓'}
                </button>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setEditandoFolio(true)}
                  className="text-[10px] text-[#1168F8] hover:underline">
                  {factura.folio ? '✏ Editar' : '+ Ingresar folio SII'}
                </button>
              </div>
            )}
          </div>
          {!factura.folio && (
            <div className="text-[10px] text-amber-600 mt-1.5">⚠ Ingresá el folio una vez que hayas emitido la factura en el portal SII</div>
          )}
        </div>
        ...
        <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100 flex-wrap">
          ...
          {(['emitida', 'enviada_sii', 'aceptada_sii', 'pagada', 'anulada'] as string[])
            .filter(e => e !== factura.estado)
            .map(e => (
              <button key={e} onClick={() => cambiarEstado(e)}
                className={`px-3 py-1 rounded-full text-[10px] font-semibold border ${ESTADO_CLS[e]} hover:opacity-80 transition-opacity`}>
                {ESTADO_L[e]}
              </button>
            ))}
        </div>
          ...
          <tfoot>
            <tr className="bg-gray-50 border-t border-gray-200">
              ...
              <td colSpan={3} className="px-4 py-2.5 text-right">
                <span className="font-mono font-black text-[#052698] text-sm">
                  {factura.moneda} {Math.round(factura.total).toLocaleString('es-CL')}
                </span>
              </td>
            </tr>
      ...
      {factura.glosa && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800">
          <span className="font-semibold">Glosa: </span>{factura.glosa}
        </div>
      )}
    </div>
  )
    ...
    <>
      ...
      <div className="no-print flex items-center justify-between mb-4">
        ...
        <button onClick={() => {
          ...
          document.title = `Factura_${factura.folio || 'borrador'}_${factura.cliente_razon_social.replace(/\s+/g, '-')}`
          window.print()
          document.title = t
        }} className="px-5 py-2.5 bg-[#052698] text-white rounded-xl text-sm font-bold hover:bg-[#1168F8]">
        ...
        <div className="factura-page bg-white">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingBottom: '12px', marginBottom: '16px', borderBottom: '3px solid #1168F8' }}>
            ...
            <div style={{ textAlign: 'right', border: '2px solid #1168F8', borderRadius: '8px', padding: '12px 20px', minWidth: '200px' }}>
              <div style={{ fontSize: '11px', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '4px' }}>
                {TIPO_DOC_L[factura.tipo_doc]}
              </div>
              <div style={{ fontSize: '28px', fontWeight: 900, fontFamily: 'monospace', color: '#052698' }}>
                N° {factura.folio || '______'}
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ padding: '6px 12px', background: '#052698', color: 'white', fontSize: '10px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>
                Emisor
              </div>
              <div style={{ padding: '10px 12px', fontSize: '11px' }}>
                ...
                <div style={{ color: '#6b7280', lineHeight: '1.6' }}>
                  Logística de importaciones<br />
                  Chile
                </div>
              </div>
            </div>
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
              <div style={{ padding: '6px 12px', background: '#1168F8', color: 'white', fontSize: '10px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>
                Cliente / Receptor
              </div>
              <div style={{ padding: '10px 12px', fontSize: '11px' }}>
          ...
          {factura.cotizacion_num && (
            <div style={{ background: '#EBF2FF', border: '1px solid #93B8FC', borderRadius: '8px', padding: '8px 12px', marginBottom: '16px', fontSize: '11px', color: '#052698' }}>
              <span style={{ fontWeight: 700 }}>Referencia operación: </span>
              <span style={{ fontFamily: 'monospace', fontWeight: 700 }}>{factura.cotizacion_num}</span>
            </div>
          )}
          ...
          <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden', marginBottom: '16px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['N°', 'Descripción', 'Cant.', 'Precio unit.', 'Descuento', factura.afecta_iva ? 'Neto' : 'Exento', factura.afecta_iva ? 'IVA' : '', 'Total'].filter(Boolean).map(h => (
                    <th key={h} style={{ padding: '8px 10px', fontSize: '9px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: '#9ca3af', borderBottom: '1px solid #e5e7eb', textAlign: h === 'Descripción' ? 'left' : 'right' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                ...
                {/* Filas vacías de reserva */}
                {items.length < 5 && Array.from({ length: 5 - items.length }).map((_, i) => (
                  <tr key={`e${i}`} style={{ borderBottom: '1px solid #f8fafc' }}>
                    <td colSpan={8} style={{ padding: '7px 10px', color: '#f1f5f9', fontSize: '11px' }}>—</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
            <div style={{ width: '260px', border: '1px solid #e5e7eb', borderRadius: '8px', overflow: 'hidden' }}>
              {factura.neto > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 12px', borderBottom: '1px solid #f1f5f9', fontSize: '11px' }}>
                  <span style={{ color: '#6b7280' }}>Neto afecto</span>
                  <span style={{ fontFamily: 'monospace', color: '#374151' }}>{fmtCLP(factura.neto)}</span>
                </div>
              )}
              {factura.exento > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 12px', borderBottom: '1px solid #f1f5f9', fontSize: '11px' }}>
                  <span style={{ color: '#6b7280' }}>Exento</span>
                  <span style={{ fontFamily: 'monospace', color: '#374151' }}>{fmtCLP(factura.exento)}</span>
                </div>
              )}
              {factura.iva_monto > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 12px', borderBottom: '1px solid #f1f5f9', fontSize: '11px', background: '#fffbeb' }}>
                  <span style={{ color: '#b45309', fontWeight: 600 }}>IVA {factura.iva_pct}%</span>
                  <span style={{ fontFamily: 'monospace', color: '#b45309', fontWeight: 600 }}>{fmtCLP(factura.iva_monto)}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 12px', background: '#052698', fontSize: '13px', fontWeight: 900 }}>
                <span style={{ color: 'white' }}>TOTAL {factura.moneda}</span>
                <span style={{ fontFamily: 'monospace', color: 'white' }}>{fmtCLP(factura.total)}</span>
              </div>
              {factura.total_usd && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 12px', background: '#EBF2FF', fontSize: '10px' }}>
                  <span style={{ color: '#052698' }}>Referencia USD (TC {fmtCLP(factura.tc_referencia || 0)})</span>
                  <span style={{ fontFamily: 'monospace', color: '#052698', fontWeight: 700 }}>USD {fmt(factura.total_usd, 0)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Glosa */}
          {factura.glosa && (
            <div style={{ border: '1px solid #fde68a', background: '#fffbeb', borderRadius: '8px', padding: '8px 12px', marginBottom: '16px', fontSize: '11px', color: '#78350f' }}>
              <span style={{ fontWeight: 700 }}>Observaciones: </span>{factura.glosa}
            </div>
          )}
          ...
          <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', fontSize: '10px', color: '#6b7280', background: '#f8fafc' }}>
            ...
            Este documento es un borrador de referencia. El documento tributario válido debe ser emitido a través del portal web del Servicio de Impuestos Internos de Chile (sii.cl) o mediante software de facturación electrónica autorizado.
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
              <div style={{ fontSize: '10px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '32px' }}>Por Puerto NOA SpA</div>
              <div style={{ borderBottom: '1px solid #9ca3af', marginBottom: '8px' }}></div>
              <div style={{ fontSize: '10px', color: '#6b7280' }}>Firma y timbre</div>
            </div>
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
              <div style={{ fontSize: '10px', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '32px' }}>Conformidad del receptor</div>
              <div style={{ borderBottom: '1px solid #9ca3af', marginBottom: '8px' }}></div>
              <div style={{ fontSize: '10px', color: '#6b7280' }}>{factura.cliente_razon_social} · Fecha: ___/___/______</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '10px', borderTop: '2px solid #1168F8' }}>
            ...
            <div style={{ textAlign: 'right', fontSize: '9px', color: '#9ca3af', fontFamily: 'monospace' }}>
              {factura.folio ? `N° ${factura.folio}` : 'Borrador'}<br />
              {factura.fecha_emision}
            </div>
          </div>
        </div>
