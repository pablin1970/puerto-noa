...
function GraficoIndexado({ series, hov, setHov }: {
  ...
  const all = series.map(s => {
    ...
    return { def: s.def, pts: s.puntos.map(p => ({ t: p.t, v: p.v, i: base ? (p.v / base) * 100 : 100 })) }
  })
  ...
  return (
    <div>
      <div className="text-xs text-gray-400 mb-3">Base 100 = primer día del rango · todos parten de 100, se ve el % que se movió cada uno.</div>
